...
课表 App 大多喜欢把功能堆在脸上——开屏广告、签到、活动、横着滚的时间轴。这也是「余课」这名字的来头:该留白的地方,就留白。
...
- **要交的、要考的也记上**。「日程」页按分类分成两份:**日程**管有时间的事(待办 / 活动),铺日历周条与按天的时间线;**倒计时**管等着倒数的目标(考试 / 作业),按「还剩几天」排。哪条要提醒、提前多久,在新建时自己勾。
...
- **桌面小组件**。今日课表 / 明日课程 / 下节课 / 日程四块,不必打开应用就能看。
...
「日程」页按分类分成两个页签——**这是两个页签唯一的分工依据**,不再只是同一份数据的两种看法:

- **日程**页签:收**待办 / 活动 / 其他**,也就是「有时间要做的事」。顶部一条日历周条(每天显示星期、日号与**农历 / 节气 / 节日**),点任意一天看那天的安排;下面按「全天 / 上午 / 下午 / 晚上」分段列出条目,每条给出起止时刻、分类与地点。点「＋」新建时默认分类是「待办」。
- **倒计时**页签:收**考试 / 作业 / 其他**,也就是「等着倒数的目标」。按「还剩几天」排序,已结束的折叠在后面(标「逾期 N 天」)。点「＋」新建时默认分类是「考试」。
- 「其他」两边都会出现。
- **新建 / 编辑**:标题、分类(待办 / 活动 / 考试 / 作业 / 其他)、全天开关、开始与结束时间、地点、备注、优先级、**提醒**。全天条目在时间线里置顶。
- **到点提醒**:每条日程可单独开启——开启后选提前量(准点 / 5 / 10 / 15 / 30 / 60 分钟);全天日程在当天设置里的固定时刻提醒(默认 09:00)。提醒与它落在哪个页签无关,一条考试只要开了照样会响。默认关闭,不设就不打扰。
...
- **日程提醒**:日程也能到点提醒。每条日程在它自己的编辑页里单独开关并选提前量;全天日程按「设置 → 提醒 → 日程提醒」里的时刻统一提醒(默认 09:00)。点击通知直达那条日程。总开关在「设置 → 提醒 → 日程提醒」。
...
- **日程**(4×2):一列近期日程与倒计时——今天的定时条目写时刻,更远的写日期,右边标「还有 N 天」;正在进行中的那条带强调色。
- 四款都可自定义显示内容(时刻 / 节次 / 地点 / 教师)、最多显示几条、行距与底色;改完立刻刷新桌面上的组件。停课 / 补课的调休日也会照实显示。
...
**日程会同步吗?**
会。日程和课表一样默认只存本机;登录同一个账号后就会一起同步(在一台设备上删除,另一台不会复活)。提醒也已经对日程生效:每条日程可在编辑页单独开关并选提前量,全天日程按「设置 → 提醒 → 日程提醒」里的时刻提醒。
