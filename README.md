...
| 周视图 | 课程 | 雨课堂公告 | 教务导入 |
| :---: | :---: | :---: | :---: |
| <img src="docs/screenshots/week.jpg" width="190" alt="周视图" /> | <img src="docs/screenshots/courses.jpg" width="190" alt="课程" /> | <img src="docs/screenshots/rain-classroom.jpg" width="190" alt="雨课堂公告" /> | <img src="docs/screenshots/import.jpg" width="190" alt="教务导入" /> |

## 它能帮上什么

- **一屏看全一周**。横着是周一到周日七列,竖着是当天所有节次,不用来回翻页;左右滑动换周,「今天」一键回到本周。
- **四种导入方式,挑顺手的用**。教务系统一键导入、手动添加单门课、粘贴表格或导入 Excel、拍张课表截图交给 AI 识别——都在周视图右上角的「添加」里。
...
- **日视图**:卡片式列表,卡片之间的距离就是空堂;每 30 秒刷新「正在上课 · 还有 N 分钟下课 / 距下一节还有 N 分钟」。日期下方是一块**当前天气**——大号温度与天气现象,配体感、湿度、风力、空气质量、紫外线,右侧是定位到的区县与今日最高最低温;按出口 IP 定位(不申请定位权限),点一下可手动刷新。
...
- **登录**:「设置 → 雨课堂」里内嵌雨课堂网页登录(扫码或账号密码皆可),登录态加密存在本机,不上传。登录通常两周左右过期,过期后重登一次即可,已配好的课程对应关系不用再改。
...
- **可整体关闭**:「设置 → 雨课堂」里有总开关,以及「课前提醒包含最新公告」「后台自动拉取」「新公告提醒」三个分开关,另有「立即刷新公告」。默认全开——没登录时它们本来就是空转的。拉取失败的原因会写在日志里(logcat tag `YuketangSync`),也可以在「高级 → 公告接口」里手动指定接口路径。
...
- **调休课表**:给某一天安排「停课」或「补课」;也可以填年份从公开接口获取全年节假日与调休,自动匹配出补课建议(逐条可改,采用前不写数据)。
- **适配器同步**:不定期更新学校与适配脚本,不必等应用发版。
- **检查更新**:自动检查的开关与频率(每天 / 每 3 天 / 每周)、是否接收预发行版(RC),都收在「关于 → 检查更新」页里。发现新版本会提示;可以「忽略此版本」,也可以随时手动检查。只提示,不自动下载。
- **使用反馈**:用起来一段时间后(装上一周、常打开、且已经有课表)会邀请你填一次问卷,只邀请一次,不想填就不再出现;之后也可以从「设置 → 关于」随时进入。

> 「AI 密钥」「课程提醒」「上课免打扰」「检查更新」都有各自的独立页面,在设置里直接点进去改。
...
> 已经装过旧版本时,直接覆盖安装即可,课表与设置都会保留。应用内「设置 → 检查更新」会认出你手机上装的是哪一种架构,只给你下对应的那一个包。
...
**节次时间和我们学校不一样?**
到「设置 → 作息时间」逐节改成你们学校的作息即可。从教务系统导入时,如果脚本读到了学校的作息,也会把「覆盖了什么」写在明面上再应用。
...
**装不上 / 装完打不开?**
多半是下载了跟你手机架构不匹配的安装包,换成 `app-universal-release.apk` 再装就好。也可以在「设置 → 检查更新」里让它自动挑——它会按你手机的架构给你对应的那一个。
